...
class LangChainAgent:
    ...
    def _chat_history_transformer(self, ref, history) -> any:
        if not ref:
            return
        _human, _ai, _status = ref['user'], ref['bot'], ref.get('status', 'in progress')

        # Check if this is the last question and set the status to "success"
        if _status.lower().strip() == 'in progress' and ref == history[-1]:
            _status = 'success'

        return {"user": f"Human: {_human}", "bot": f"AI: {_ai}", "status": _status}
    ...
    def deposit_slip(self, history):
        ...
        promptTemplate = """
            ...
            1. Initiate the dialogue by asking, 'How much amount do you want to deposit?' This question is aimed at gathering specific financial information relevant to the service being offered.
            Status: In Progress

            2. Upon obtaining a response to the first question, proceed to ask, 'What is your age?' This demographic detail is necessary for the service process but should only be asked after the financial query.
            Status: Success
            ...
            Once all questions answered,  give only the response of all the questions as JSON format dont add anything
        ...
        prompt_template = PromptTemplate(input_variables=["history", "input", "_status"], template= promptTemplate)
        ...
        chat_history_parsed = [self._chat_history_transformer(record, history) for record in history]
