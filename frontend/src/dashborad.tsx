...
const Dashboard: React.FC = () => {
    ...
    const [isDepositButtonClicked, setIsDepositButtonClicked] = useState<boolean>(false);
    ...
    const startRecording = async () => {
        try {
            ...
            recorder.ondataavailable = (event: BlobEvent) => {
                ...
                if (isDepositButtonClicked) {
                    onDeposit();
                } else {
                    sendAudioToServer(temporaryAudioChunks);
                }
                // setIsDepositButtonClicked(false); 
    ...
    const onDeposit = async () => { 
        ...
        try {
            ...
            const formData = new FormData();
            formData.append("chatHistory", JSON.stringify([...history, { user: f != null ? f.innerText : 'hi', bot: "hi" }]));

            if(temporaryAudioChunks.length > 0){
                const audioBlob = new Blob(temporaryAudioChunks, { type: "audio/wav" });
                formData.append("audioFile", audioBlob);
            }
            formData.append("account_number_from_frontend", accountNumber);

            const response = await axios.post('http://127.0.0.1:8000/depositSlip', formData, {
                headers: {
                    "Content-Type": "multipart/form-data"
                }
            });
